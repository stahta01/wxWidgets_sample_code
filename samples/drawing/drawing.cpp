...
bool MyApp::LoadImages()
{
    ...
    wxString path = pathList.FindValidPath(wxT_2("pat4.bmp"));
    if ( !path )
        return false;
    ...
    path = pathList.FindValidPath(wxT_2("pat36.bmp"));
    if ( !path )
        return false;
    ...
    path = pathList.FindValidPath(wxT_2("image.bmp"));
    if ( !path )
        return false;
    ...
    path = pathList.FindValidPath(wxT_2("mask.bmp"));
    if ( !path )
        return false;
...
bool MyApp::OnInit()
{
    ...
    MyFrame *frame = new MyFrame(wxS("Drawing sample"),
    ...
    if ( !LoadImages() )
    {
        wxLogError(wxT_2("Can't load one of the bitmap files needed ")
                   wxT_2("for this sample from the current or parent ")
                   wxT_2("directory, please copy them there."));
...
void MyCanvas::DrawTestBrushes(wxDC& dc)
{
    ...
    dc.DrawText(wxT_2("Solid green"), x + 10, y + 10);

    y += HEIGHT;
    dc.SetBrush(wxBrush(*wxRED, wxBRUSHSTYLE_CROSSDIAG_HATCH));
    dc.DrawRectangle(x, y, WIDTH, HEIGHT);
    dc.DrawText(wxT_2("Diagonally hatched red"), x + 10, y + 10);

    y += HEIGHT;
    dc.SetBrush(wxBrush(*wxBLUE, wxBRUSHSTYLE_CROSS_HATCH));
    dc.DrawRectangle(x, y, WIDTH, HEIGHT);
    dc.DrawText(wxT_2("Cross hatched blue"), x + 10, y + 10);

    y += HEIGHT;
    dc.SetBrush(wxBrush(*wxCYAN, wxBRUSHSTYLE_VERTICAL_HATCH));
    dc.DrawRectangle(x, y, WIDTH, HEIGHT);
    dc.DrawText(wxT_2("Vertically hatched cyan"), x + 10, y + 10);

    y += HEIGHT;
    dc.SetBrush(wxBrush(*wxBLACK, wxBRUSHSTYLE_HORIZONTAL_HATCH));
    dc.DrawRectangle(x, y, WIDTH, HEIGHT);
    dc.DrawText(wxT_2("Horizontally hatched black"), x + 10, y + 10);

    y += HEIGHT;
    dc.SetBrush(wxBrush(*gs_bmpMask));
    dc.DrawRectangle(x, y, WIDTH, HEIGHT);
    dc.DrawText(wxT_2("Stipple mono"), x + 10, y + 10);

    y += HEIGHT;
    dc.SetBrush(wxBrush(*gs_bmpNoMask));
    dc.DrawRectangle(x, y, WIDTH, HEIGHT);
    dc.DrawText(wxT_2("Stipple colour"), x + 10, y + 10);
}
...
void MyCanvas::DrawTestPoly(wxDC& dc)
{
    ...
    dc.DrawText(wxT_2("You should see two (irregular) stars below, the left one ")
                wxT_2("hatched"), 10, 10);
    dc.DrawText(wxT_2("except for the central region and the right ")
                wxT_2("one entirely hatched"), 10, 30);
    dc.DrawText(wxT_2("The third star only has a hatched outline"), 10, 50);
...
void MyCanvas::DrawTestLines( int x, int y, int width, wxDC &dc )
{
    ...
    dc.DrawText(wxString::Format(wxT_2("Testing lines of width %d"), width), x + 10, y - 10);
    dc.DrawRectangle( x+10, y+10, 100, 190 );

    dc.DrawText(wxT_2("Solid/dot/short dash/long dash/dot dash"), x + 150, y + 10);
    dc.SetPen( wxPen( *wxBLACK, width ) );
    ...
    dc.DrawText(wxT_2("Misc hatches"), x + 150, y + 70);
    ...
    dc.DrawText(wxT_2("User dash"), x + 150, y + 140);
...
void MyCanvas::DrawText(wxDC& dc)
{
    ...
    dc.DrawText( wxT_2("This is text"), 110, 10 );
    dc.DrawRotatedText( wxT_2("That is text"), 20, 10, -45 );
    ...
    {
        text.Printf(wxT_2("     %d rotated text"), n);
        dc.DrawRotatedText(text , 400, 400, n);
    }

    dc.SetFont( wxFontInfo(18).Family(wxFONTFAMILY_SWISS) );

    dc.DrawText( wxT_2("This is Swiss 18pt text."), 110, 40 );

    wxCoord length;
    wxCoord height;
    wxCoord descent;
    dc.GetTextExtent( wxT_2("This is Swiss 18pt text."), &length, &height, &descent );
    text.Printf( wxT_2("Dimensions are length %d, height %d, descent %d"), length, height, descent );
    dc.DrawText( text, 110, 80 );

    text.Printf( wxT_2("CharHeight() returns: %d"), dc.GetCharHeight() );
    ...
    dc.DrawText( wxT_2("There should be a text below"), 110, y );
    dc.DrawRectangle( 110, y, 100, height );

    y += height;
    dc.DrawText( wxT_2("Visible text"), 110, y );
    dc.DrawRectangle( 110, y, 100, height );
    dc.DrawText( wxT_2("Visible text"), 110, y );
    dc.DrawRectangle( 110, y, 100, height );
    ...
    y += height;
    dc.DrawRectangle( 110, y, 100, height );
    dc.DrawText( wxT_2("Another visible text"), 110, y );
...
{
    { wxS("wxAND"),          wxAND           },
    { wxS("wxAND_INVERT"),   wxAND_INVERT    },
    { wxS("wxAND_REVERSE"),  wxAND_REVERSE   },
    { wxS("wxCLEAR"),        wxCLEAR         },
    { wxS("wxCOPY"),         wxCOPY          },
    { wxS("wxEQUIV"),        wxEQUIV         },
    { wxS("wxINVERT"),       wxINVERT        },
    { wxS("wxNAND"),         wxNAND          },
    { wxS("wxNO_OP"),        wxNO_OP         },
    { wxS("wxOR"),           wxOR            },
    { wxS("wxOR_INVERT"),    wxOR_INVERT     },
    { wxS("wxOR_REVERSE"),   wxOR_REVERSE    },
    { wxS("wxSET"),          wxSET           },
    { wxS("wxSRC_INVERT"),   wxSRC_INVERT    },
    { wxS("wxXOR"),          wxXOR           },
};

void MyCanvas::DrawImages(wxDC& dc, DrawMode mode)
{
    dc.DrawText(wxT_2("original image"), 0, 0);
    dc.DrawBitmap(*gs_bmpNoMask, 0, 20, 0);
    dc.DrawText(wxT_2("with colour mask"), 0, 100);
    dc.DrawBitmap(*gs_bmpWithColMask, 0, 120, true);
    dc.DrawText(wxT_2("the mask image"), 0, 200);
    dc.DrawBitmap(*gs_bmpMask, 0, 220, 0);
    dc.DrawText(wxT_2("masked image"), 0, 300);
...
void MyCanvas::DrawAlpha(wxDC& dc)
{
    ...
    dc.DrawText( wxT_2("Hello!"), 120, 80 );
}
#endif // wxDRAWING_DC_SUPPORTS_ALPHA || wxUSE_GRAPHICS_CONTEXT
...
{
#if wxUSE_SPLINES
    dc.DrawText(wxT_2("Some splines"), 10, 5);
...
#else
    dc.DrawText(wxT_2("Splines not supported."), 10, 5);
#endif
}
...
void MyCanvas::DrawGradients(wxDC& dc)
{
    ...
    dc.DrawText(wxT_2("Blue inside"), r.x, r.y);
    r.Offset(0, TEXT_HEIGHT);
    dc.GradientFillConcentric(r, *wxBLUE, *wxWHITE);

    r.Offset(0, r.height + 10);
    dc.DrawText(wxT_2("White inside"), r.x, r.y);
    r.Offset(0, TEXT_HEIGHT);
    dc.GradientFillConcentric(r, *wxWHITE, *wxBLUE);

    r.Offset(0, r.height + 10);
    dc.DrawText(wxT_2("Blue in top left corner"), r.x, r.y);
    r.Offset(0, TEXT_HEIGHT);
    dc.GradientFillConcentric(r, *wxBLUE, *wxWHITE, wxPoint(0, 0));

    r.Offset(0, r.height + 10);
    dc.DrawText(wxT_2("Blue in bottom right corner"), r.x, r.y);
    r.Offset(0, TEXT_HEIGHT);
    ...
    if (m_renderer)
    {
        ...
        dc.DrawText(wxT_2("Linear Gradient with Stops"), gfr.x, gfr.y);
        ...
        gfr.Offset(0, gfr.height + 10);
        dc.DrawText(wxT_2("Radial Gradient from Red to Blue without intermediary Stops"),
            gfr.x, gfr.y);
        gfr.Offset(0, TEXT_HEIGHT);
        ...
        gfr.Offset(0, gfr.height + 10);
        dc.DrawText(wxT_2("Radial Gradient from Red to Blue with Yellow and Green Stops"),
            gfr.x, gfr.y);
        gfr.Offset(0, TEXT_HEIGHT);
        ...
        gfr.Offset(0, gfr.height + 10);
        dc.DrawText(wxT_2("Linear Gradient with Stops and Gaps"), gfr.x, gfr.y);
        gfr.Offset(0, TEXT_HEIGHT);
        ...
        gfr.Offset(0, gfr.height + 10);
        dc.DrawText(wxT_2("Radial Gradient with Stops and Gaps"), gfr.x, gfr.y);
        gfr.Offset(0, TEXT_HEIGHT);
        ...
        gfr.Offset(0, gfr.height + 10);
        dc.DrawText(wxT_2("Gradients with Stops and Transparency"), gfr.x, gfr.y);
        gfr.Offset(0, TEXT_HEIGHT);
...
{
    dc.DrawText(wxT_2("You should see a red rect partly covered by a cyan one ")
                wxT_2("on the left"), 10, 5);
    dc.DrawText(wxT_2("and 5 smileys from which 4 are partially clipped on the right"),
                10, 5 + dc.GetCharHeight());
    dc.DrawText(wxT_2("The second copy should be identical but right part of it ")
                wxT_2("should be offset by 10 pixels."),
...
void MyCanvas::OnMouseMove(wxMouseEvent &event)
{
#if wxUSE_STATUSBAR
    {
        ...
        wxString str;
        str.Printf( wxT_2("Current mouse position: %d,%d"), (int)x, (int)y );
                 ...
                 wxDEFAULT_FRAME_STYLE | wxNO_FULL_REPAINT_ON_RESIZE)
{
    ...
    menuScreen->Append(File_ShowDefault, wxT_2("&Default screen\tF1"));
    menuScreen->Append(File_ShowText, wxT_2("&Text screen\tF2"));
    menuScreen->Append(File_ShowLines, wxT_2("&Lines screen\tF3"));
    menuScreen->Append(File_ShowBrushes, wxT_2("&Brushes screen\tF4"));
    menuScreen->Append(File_ShowPolygons, wxT_2("&Polygons screen\tF5"));
    menuScreen->Append(File_ShowMask, wxT_2("&Mask screen\tF6"));
    menuScreen->Append(File_ShowMaskStretch, wxT_2("1/&2 scaled mask\tShift-F6"));
    menuScreen->Append(File_ShowOps, wxT_2("&Raster operations screen\tF7"));
    menuScreen->Append(File_ShowRegions, wxT_2("Re&gions screen\tF8"));
    menuScreen->Append(File_ShowCircles, wxT_2("&Circles screen\tF9"));
#if wxDRAWING_DC_SUPPORTS_ALPHA || wxUSE_GRAPHICS_CONTEXT
    menuScreen->Append(File_ShowAlpha, wxT_2("&Alpha screen\tF10"));
#endif // wxDRAWING_DC_SUPPORTS_ALPHA || wxUSE_GRAPHICS_CONTEXT
    menuScreen->Append(File_ShowSplines, wxT_2("Spl&ines screen\tF11"));
    menuScreen->Append(File_ShowGradients, wxT_2("&Gradients screen\tF12"));
#if wxUSE_GRAPHICS_CONTEXT
    menuScreen->Append(File_ShowGraphics, wxT_2("&Graphics screen"));
#endif
    menuScreen->Append(File_ShowSystemColours, wxT_2("System &colours"));
    ...
    menuFile->AppendCheckItem(File_Clip, wxT_2("&Clip\tCtrl-C"), wxT_2("Clip/unclip drawing"));
    menuFile->AppendCheckItem(File_Buffer, wxT_2("&Use wx&BufferedPaintDC\tCtrl-Z"), wxT_2("Buffer painting"));
    menuFile->AppendSeparator();
#if wxUSE_METAFILE && defined(wxMETAFILE_IS_ENH)
    menuFile->Append(File_Copy, wxT_2("Copy to clipboard"));
#endif
    menuFile->Append(File_Save, wxT_2("&Save...\tCtrl-S"), wxT_2("Save drawing to file"));
    menuFile->AppendSeparator();
    menuFile->Append(File_About, wxT_2("&About\tCtrl-A"), wxT_2("Show about dialog"));
    menuFile->AppendSeparator();
    menuFile->Append(File_Quit, wxT_2("E&xit\tAlt-X"), wxT_2("Quit this program"));

    wxMenu *menuMapMode = new wxMenu;
    menuMapMode->Append( MapMode_Text, wxT_2("&TEXT map mode") );
    menuMapMode->Append( MapMode_Lometric, wxT_2("&LOMETRIC map mode") );
    menuMapMode->Append( MapMode_Twips, wxT_2("T&WIPS map mode") );
    menuMapMode->Append( MapMode_Points, wxT_2("&POINTS map mode") );
    menuMapMode->Append( MapMode_Metric, wxT_2("&METRIC map mode") );

    wxMenu *menuUserScale = new wxMenu;
    menuUserScale->Append( UserScale_StretchHoriz, wxT_2("Stretch &horizontally\tCtrl-H") );
    menuUserScale->Append( UserScale_ShrinkHoriz, wxT_2("Shrin&k horizontally\tCtrl-G") );
    menuUserScale->Append( UserScale_StretchVertic, wxT_2("Stretch &vertically\tCtrl-V") );
    menuUserScale->Append( UserScale_ShrinkVertic, wxT_2("&Shrink vertically\tCtrl-W") );
    menuUserScale->AppendSeparator();
    menuUserScale->Append( UserScale_Restore, wxT_2("&Restore to normal\tCtrl-0") );

    wxMenu *menuAxis = new wxMenu;
    menuAxis->AppendCheckItem( AxisMirror_Horiz, wxT_2("Mirror horizontally\tCtrl-M") );
    menuAxis->AppendCheckItem( AxisMirror_Vertic, wxT_2("Mirror vertically\tCtrl-N") );

    wxMenu *menuLogical = new wxMenu;
    menuLogical->Append( LogicalOrigin_MoveDown, wxT_2("Move &down\tCtrl-D") );
    menuLogical->Append( LogicalOrigin_MoveUp, wxT_2("Move &up\tCtrl-U") );
    menuLogical->Append( LogicalOrigin_MoveLeft, wxT_2("Move &right\tCtrl-L") );
    menuLogical->Append( LogicalOrigin_MoveRight, wxT_2("Move &left\tCtrl-R") );
    menuLogical->AppendSeparator();
    menuLogical->Append( LogicalOrigin_Set, wxT_2("Set to (&100, 100)\tShift-Ctrl-1") );
    menuLogical->Append( LogicalOrigin_Restore, wxT_2("&Restore to normal\tShift-Ctrl-0") );

    wxMenu *menuColour = new wxMenu;
#if wxUSE_COLOURDLG
    menuColour->Append( Colour_TextForeground, wxT_2("Text &foreground...") );
    menuColour->Append( Colour_TextBackground, wxT_2("Text &background...") );
    menuColour->Append( Colour_Background, wxT_2("Background &colour...") );
#endif // wxUSE_COLOURDLG
    menuColour->AppendCheckItem( Colour_BackgroundMode, wxT_2("&Opaque/transparent\tCtrl-B") );
    menuColour->AppendCheckItem( Colour_TextureBackgound, wxT_2("Draw textured back&ground\tCtrl-T") );

    // now append the freshly created menu to the menu bar...
    wxMenuBar *menuBar = new wxMenuBar;
    menuBar->Append(menuFile, wxT_2("&Drawing"));
    menuBar->Append(menuScreen, wxT_2("Scree&n"));
    menuBar->Append(menuMapMode, wxT_2("&Mode"));
    menuBar->Append(menuUserScale, wxT_2("&Scale"));
    menuBar->Append(menuAxis, wxT_2("&Axis"));
    menuBar->Append(menuLogical, wxT_2("&Origin"));
    menuBar->Append(menuColour, wxT_2("&Colours"));
...
#if wxUSE_STATUSBAR
    CreateStatusBar(2);
    SetStatusText(wxT_2("Welcome to wxWidgets!"));
#endif // wxUSE_STATUSBAR
...
{
    wxString msg;
    msg.Printf( wxT_2("This is the about dialog of the drawing sample.\n")
                wxT_2("This sample tests various primitive drawing functions\n")
                wxT_2("(without any attempts to prevent flicker).\n")
                wxT_2("Copyright (c) Robert Roebling 1999")
              );

    wxMessageBox(msg, wxT_2("About Drawing"), wxOK | wxICON_INFORMATION, this);
}
...
void MyFrame::OnSave(wxCommandEvent& WXUNUSED(event))
{
    ...
    wxFileDialog dlg(this, wxT_2("Save as bitmap"), wxEmptyString, wxEmptyString,
                     wildCard,
    ...
    if (dlg.ShowModal() == wxID_OK)
    {
        ...
        if (ext == wxT_2("svg"))
        {
            ...
            m_canvas->UseGraphicRenderer(NULL);
#endif
            wxSVGFileDC svgdc(dlg.GetPath(), width, height, 72, wxT_2("Drawing sample"));
