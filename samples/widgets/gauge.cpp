...
IMPLEMENT_WIDGETS_PAGE(GaugeWidgetsPage, wxS("Gauge"), FAMILY_CTRLS );
...
void GaugeWidgetsPage::CreateContent()
{
    ...
    m_chkSmooth = CreateCheckBoxAndAddToSizer(sizerLeft, wxT_2("&Smooth"));
    m_chkProgress = CreateCheckBoxAndAddToSizer(sizerLeft, wxT_2("&Progress"));

    sizerLeft->Add(5, 5, 0, wxGROW | wxALL, 5); // spacer

    wxButton *btn = new wxButton(this, GaugePage_Reset, wxT_2("&Reset"));
    sizerLeft->Add(btn, 0, wxALIGN_CENTRE_HORIZONTAL | wxALL, 15);

    // middle pane
    wxStaticBox *box2 = new wxStaticBox(this, wxID_ANY, wxT_2("&Change gauge value"));
    ...
    sizerRow = CreateSizerWithTextAndButton(GaugePage_SetRange,
                                            ...
                                            GaugePage_RangeText,
                                            &m_textRange);
    m_textRange->SetValue( wxString::Format(wxT_2("%lu"), m_range) );
    sizerMiddle->Add(sizerRow, 0, wxALL | wxGROW, 5);

    btn = new wxButton(this, GaugePage_Progress, wxT_2("Simulate &progress"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);

    btn = new wxButton(this, GaugePage_IndeterminateProgress,
                       wxT_2("Simulate &indeterminate job"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);

    btn = new wxButton(this, GaugePage_Clear, wxT_2("&Clear"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);
...
void GaugeWidgetsPage::StartTimer(wxButton *clicked)
{
    ...
    wxLogMessage(wxT_2("Launched progress timer (interval = %d ms)"), INTERVAL);

    m_timer = new wxTimer(this,
        clicked->GetId() == GaugePage_Progress ? GaugePage_Timer : GaugePage_IndeterminateTimer);
    m_timer->Start(INTERVAL);

    clicked->SetLabel(wxT_2("&Stop timer"));
...
{
    wxCHECK_RET( m_timer, wxT_2("shouldn't be called") );
    ...
    if (clicked->GetId() == GaugePage_Progress)
    {
        clicked->SetLabel(wxT_2("Simulate &progress"));
        FindWindow(GaugePage_IndeterminateProgress)->Enable();
    }
    else
    {
        clicked->SetLabel(wxT_2("Simulate indeterminate job"));
        FindWindow(GaugePage_Progress)->Enable();
    }

    wxLogMessage(wxT_2("Progress finished."));
}
...
void GaugeWidgetsPage::OnButtonProgress(wxCommandEvent& event)
{
    ...
    else // stop the running timer
    {
        ...
        wxLogMessage(wxT_2("Stopped the timer."));
    }
}
...
void GaugeWidgetsPage::OnProgressTimer(wxTimerEvent& WXUNUSED(event))
{
    ...
    else // reached the end
    {
        ...
        wxCHECK_RET( btn, wxT_2("no progress button?") );
...
{
    event.SetText( wxString::Format(wxT_2("%d"), m_gauge->GetValue()));
}
