...
IMPLEMENT_WIDGETS_PAGE(ODComboboxWidgetsPage, wxS("OwnerDrawnCombobox"),
...
void ODComboboxWidgetsPage::CreateContent()
{
    ...
    */
    ...
    // left pane - style box
    wxStaticBox *box = new wxStaticBox(this, wxID_ANY, wxT_2("&Set style"));

    wxSizer *sizerStyle = new wxStaticBoxSizer(box, wxVERTICAL);

    m_chkSort = CreateCheckBoxAndAddToSizer(sizerStyle, wxT_2("&Sort items"));
    m_chkReadonly = CreateCheckBoxAndAddToSizer(sizerStyle, wxT_2("&Read only"));
    m_chkDclickcycles = CreateCheckBoxAndAddToSizer(sizerStyle, wxT_2("&Double-click Cycles"));

    sizerStyle->AddSpacer(4);

    m_chkBitmapbutton = CreateCheckBoxAndAddToSizer(sizerStyle, wxT_2("&Bitmap button"));
    m_chkStdbutton = CreateCheckBoxAndAddToSizer(sizerStyle, wxT_2("B&lank button background"));

    wxButton *btn = new wxButton(this, ODComboPage_Reset, wxT_2("&Reset"));
    ...
    box = new wxStaticBox(this, wxID_ANY, wxT_2("Adjust &popup"));

    wxSizer *sizerPopupPos = new wxStaticBoxSizer(box, wxVERTICAL);

    sizerRow = CreateSizerWithTextAndLabel(wxT_2("Min. Width:"),
                                           ODComboPage_PopupMinWidth,
                                           &m_textPopupMinWidth);
    m_textPopupMinWidth->SetValue(wxT_2("-1"));
    sizerPopupPos->Add(sizerRow, 0, wxALL | wxGROW, 5);

    sizerRow = CreateSizerWithTextAndLabel(wxT_2("Max. Height:"),
                                           ODComboPage_PopupHeight,
                                           &m_textPopupHeight);
    m_textPopupHeight->SetValue(wxT_2("-1"));
    sizerPopupPos->Add(sizerRow, 0, wxALL | wxGROW, 5);

    m_chkAlignpopupright = CreateCheckBoxAndAddToSizer(sizerPopupPos, wxT_2("Align Right"));

    sizerLeft->Add(sizerPopupPos, wxSizerFlags().Expand().Border(wxTOP, 2));

    // left pane - button adjustment box
    box = new wxStaticBox(this, wxID_ANY, wxT_2("Adjust &button"));

    wxSizer *sizerButtonPos = new wxStaticBoxSizer(box, wxVERTICAL);

    sizerRow = CreateSizerWithTextAndLabel(wxT_2("Width:"),
                                           ODComboPage_ButtonWidth,
                                           &m_textButtonWidth);
    m_textButtonWidth->SetValue(wxT_2("-1"));
    sizerButtonPos->Add(sizerRow, 0, wxALL | wxGROW, 5);

    sizerRow = CreateSizerWithTextAndLabel(wxT_2("VSpacing:"),
                                           ODComboPage_ButtonSpacing,
                                           &m_textButtonSpacing);
    m_textButtonSpacing->SetValue(wxT_2("0"));
    sizerButtonPos->Add(sizerRow, 0, wxALL | wxGROW, 5);

    sizerRow = CreateSizerWithTextAndLabel(wxT_2("Height:"),
                                           ODComboPage_ButtonHeight,
                                           &m_textButtonHeight);
    m_textButtonHeight->SetValue(wxT_2("-1"));
    sizerButtonPos->Add(sizerRow, 0, wxALL | wxGROW, 5);

    m_chkAlignbutleft = CreateCheckBoxAndAddToSizer(sizerButtonPos, wxT_2("Align Left"));

    sizerLeft->Add(sizerButtonPos, wxSizerFlags().Expand().Border(wxTOP, 2));

    // middle pane
    wxStaticBox *box2 = new wxStaticBox(this, wxID_ANY,
        wxT_2("&Change combobox contents"));
    wxSizer *sizerMiddle = new wxStaticBoxSizer(box2, wxVERTICAL);

    btn = new wxButton(this, ODComboPage_ContainerTests, wxT_2("Run &tests"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);

    sizerRow = CreateSizerWithTextAndLabel(wxT_2("Current selection"),
                                           ODComboPage_CurText,
                                           &text);
    ...
    sizerRow = CreateSizerWithTextAndLabel(wxT_2("Insertion Point"),
                                           ODComboPage_InsertionPointText,
                                           &text);
    text->SetEditable(false);
    ...
    sizerRow = CreateSizerWithTextAndButton(ODComboPage_Insert,
                                            wxT_2("&Insert this string"),
                                            ODComboPage_InsertText,
                                            &m_textInsert);
    sizerMiddle->Add(sizerRow, 0, wxALL | wxGROW, 5);

    sizerRow = CreateSizerWithTextAndButton(ODComboPage_Add,
                                            wxT_2("&Add this string"),
                                            ODComboPage_AddText,
                                            &m_textAdd);
    sizerMiddle->Add(sizerRow, 0, wxALL | wxGROW, 5);

    btn = new wxButton(this, ODComboPage_AddSeveral, wxT_2("&Append a few strings"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);

    btn = new wxButton(this, ODComboPage_AddMany, wxT_2("Append &many strings"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);

    sizerRow = CreateSizerWithTextAndButton(ODComboPage_Change,
                                            wxT_2("C&hange current"),
                                            ODComboPage_ChangeText,
                                            &m_textChange);
    sizerMiddle->Add(sizerRow, 0, wxALL | wxGROW, 5);

    sizerRow = CreateSizerWithTextAndButton(ODComboPage_Delete,
                                            wxT_2("&Delete this item"),
                                            ODComboPage_DeleteText,
                                            &m_textDelete);
    sizerMiddle->Add(sizerRow, 0, wxALL | wxGROW, 5);

    btn = new wxButton(this, ODComboPage_DeleteSel, wxT_2("Delete &selection"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);

    btn = new wxButton(this, ODComboPage_Clear, wxT_2("&Clear"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);
...
#else
        wxLogMessage(wxT_2("Not implemented in wxGTK"));
#endif
    }
}
...
void ODComboboxWidgetsPage::OnButtonInsert(wxCommandEvent& WXUNUSED(event))
{
    ...
    if ( !m_textInsert->IsModified() )
    {
        ...
        m_textInsert->SetValue(wxString::Format(wxT_2("test item %u"), ++s_item));
    }
...
void ODComboboxWidgetsPage::OnButtonAdd(wxCommandEvent& WXUNUSED(event))
{
    ...
    {
        // update the default string
        m_textAdd->SetValue(wxString::Format(wxT_2("test item %u"), ++s_item));
    }
...
void ODComboboxWidgetsPage::OnButtonAddMany(wxCommandEvent& WXUNUSED(event))
{
    ...
    {
        m_combobox->Append(wxString::Format(wxT_2("item #%u"), n));
    }
}

void ODComboboxWidgetsPage::OnButtonAddSeveral(wxCommandEvent& WXUNUSED(event))
{
    m_combobox->Append(wxT_2("First"));
    m_combobox->Append(wxT_2("another one"));
    m_combobox->Append(wxT_2("and the last (very very very very very very very very very very long) one"));
}
...
{
    if (m_combobox)
        event.SetText( wxString::Format(wxT_2("%d"), m_combobox->GetSelection()) );
}

void ODComboboxWidgetsPage::OnUpdateUIInsertionPointText(wxUpdateUIEvent& event)
{
    if (m_combobox)
        event.SetText( wxString::Format(wxT_2("%ld"), m_combobox->GetInsertionPoint()) );
}
...
void ODComboboxWidgetsPage::OnComboText(wxCommandEvent& event)
{
    ...
    wxASSERT_MSG( s == m_combobox->GetValue(),
                  wxT_2("event and combobox values should be the same") );

    if (event.GetEventType() == wxEVT_TEXT_ENTER)
    {
        wxLogMessage(wxT_2("OwnerDrawnCombobox enter pressed (now '%s')"), s.c_str());
    }
    else
    {
        wxLogMessage(wxT_2("OwnerDrawnCombobox text changed (now '%s')"), s.c_str());
    }
}
...
void ODComboboxWidgetsPage::OnComboBox(wxCommandEvent& event)
{
    ...
    m_textDelete->SetValue(wxString::Format(wxT_2("%ld"), sel));

    wxLogMessage(wxT_2("OwnerDrawnCombobox item %ld selected"), sel);

    wxLogMessage(wxT_2("OwnerDrawnCombobox GetValue(): %s"), m_combobox->GetValue().c_str() );
}
...
{
    wxLogMessage(wxT_2("Combobox dropped down"));
}

void ODComboboxWidgetsPage::OnCloseUp(wxCommandEvent& WXUNUSED(event))
{
    wxLogMessage(wxT_2("Combobox closed up"));
}
