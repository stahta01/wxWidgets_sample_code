...
void BookWidgetsPage::CreateContent()
{
    ...
    orientations.Add(wxT_2("&top"));
    orientations.Add(wxT_2("&bottom"));
    orientations.Add(wxT_2("&left"));
    orientations.Add(wxT_2("&right"));

    wxASSERT_MSG( orientations.GetCount() == Orient_Max,
                  wxT_2("forgot to update something") );

    m_chkImages = new wxCheckBox(this, wxID_ANY, wxT_2("Show &images"));
    m_radioOrient = new wxRadioBox(this, wxID_ANY, wxT_2("&Tab orientation"),
    ...
    wxButton *btn = new wxButton(this, BookPage_Reset, wxT_2("&Reset"));
    sizerLeft->Add(btn, 0, wxALIGN_CENTRE_HORIZONTAL | wxALL, 15);

    // middle pane
    wxStaticBox *box2 = new wxStaticBox(this, wxID_ANY, wxT_2("&Contents"));
    wxSizer *sizerMiddle = new wxStaticBoxSizer(box2, wxVERTICAL);

    wxTextCtrl *text;
    wxSizer *sizerRow = CreateSizerWithTextAndLabel(wxT_2("Number of pages: "),
                                                    BookPage_NumPagesText,
                                                    &text);
    ...
    btn = new wxButton(this, BookPage_AddPage, wxT_2("&Add page"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);

    sizerRow = CreateSizerWithTextAndButton(BookPage_InsertPage,
                                            wxT_2("&Insert page at"),
                                            BookPage_InsertText,
                                            &m_textInsert);
    sizerMiddle->Add(sizerRow, 0, wxALL | wxGROW, 5);
    ...
    btn = new wxButton(this, BookPage_DeleteAll, wxT_2("&Delete All"));
    sizerMiddle->Add(btn, 0, wxALL | wxGROW, 5);
...
void BookWidgetsPage::RecreateBook()
{
    ...
    {
        default:
            wxFAIL_MSG( wxT_2("unknown orientation") );
            // fall through
...
{
    return new wxTextCtrl(m_book, wxID_ANY, wxT_2("I'm a book page"));
}
...
void BookWidgetsPage::OnButtonSelectPage(wxCommandEvent& WXUNUSED(event))
{
    ...
    wxCHECK_RET( IsValidValue(pos), wxT_2("button should be disabled") );
...
{
    m_book->AddPage(CreateNewPage(), wxT_2("Added page"), false,
                    GetIconIndex());
}
...
void BookWidgetsPage::OnButtonInsertPage(wxCommandEvent& WXUNUSED(event))
{
    ...
    m_book->InsertPage(pos, CreateNewPage(), wxT_2("Inserted page"), false,
                       GetIconIndex());
}
...
{
    if(m_book)
        event.SetText( wxString::Format(wxT_2("%u"), unsigned(m_book->GetPageCount())) );
}

void BookWidgetsPage::OnUpdateUICurSelectText(wxUpdateUIEvent& event)
{
    if(m_book)
        event.SetText( wxString::Format(wxT_2("%d"), m_book->GetSelection()) );
}
...
IMPLEMENT_WIDGETS_PAGE(NotebookWidgetsPage, wxS("Notebook"),
                       FAMILY_CTRLS | BOOK_CTRLS
                       );

void NotebookWidgetsPage::OnPageChanging(wxNotebookEvent& event)
{
    wxLogMessage(wxT_2("Notebook page changing from %d to %d (currently %d)."),
...
{
    wxLogMessage(wxT_2("Notebook page changed from %d to %d (currently %d)."),
                 event.GetOldSelection(),
                 event.GetSelection(),
                 m_book->GetSelection());
...
IMPLEMENT_WIDGETS_PAGE(ListbookWidgetsPage, wxS("Listbook"),
                       GENERIC_CTRLS | BOOK_CTRLS
                       );

void ListbookWidgetsPage::OnPageChanging(wxListbookEvent& event)
{
    wxLogMessage(wxT_2("Listbook page changing from %d to %d (currently %d)."),
                 event.GetOldSelection(),
                 event.GetSelection(),
                 m_book->GetSelection());
...
{
    wxLogMessage(wxT_2("Listbook page changed from %d to %d (currently %d)."),
                 event.GetOldSelection(),
                 event.GetSelection(),
                 m_book->GetSelection());
...
IMPLEMENT_WIDGETS_PAGE(ChoicebookWidgetsPage, wxS("Choicebook"),
                       GENERIC_CTRLS | BOOK_CTRLS
                       );

void ChoicebookWidgetsPage::OnPageChanging(wxChoicebookEvent& event)
{
    wxLogMessage(wxT_2("Choicebook page changing from %d to %d (currently %d)."),
                 event.GetOldSelection(),
                 event.GetSelection(),
                 m_book->GetSelection());
...
{
    wxLogMessage(wxT_2("Choicebook page changed from %d to %d (currently %d)."),
                 event.GetOldSelection(),
                 event.GetSelection(),
                 m_book->GetSelection());
