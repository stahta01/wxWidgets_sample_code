...
#if wxUSE_HELP
    panel->SetHelpText( wxT_2( "Panel with a Button" ) );
#endif

    (void) new wxButton( panel, wxID_ANY, wxT_2("Button"),
...
#if wxUSE_HELP
    panel->SetHelpText( wxT_2( "Panel with some Radio Buttons" ) );
#endif

    wxString animals[] =
      { wxT_2("Fox"), wxT_2("Hare"), wxT_2("Rabbit"),
        wxT_2("Sabre-toothed tiger"), wxT_2("T Rex") };

    wxRadioBox *radiobox1 = new wxRadioBox(panel, wxID_ANY, wxT_2("Choose one"),
        wxDefaultPosition, wxDefaultSize, 5, animals, 2, wxRA_SPECIFY_ROWS);

    wxString computers[] =
      { wxT_2("Amiga"), wxT_2("Commodore 64"), wxT_2("PET"),
        wxT_2("Another") };

    wxRadioBox *radiobox2 = new wxRadioBox(panel, wxID_ANY,
        wxT_2("Choose your favourite"), wxDefaultPosition, wxDefaultSize,
...
#if wxUSE_HELP
    panel->SetHelpText( wxT_2( "An empty panel" ) );
#endif

    (void) new wxStaticText( panel, wxID_ANY,
                             wxT_2("This page intentionally left blank"),
                             wxPoint(10, 10) );
...
#if wxUSE_HELP
    panel->SetHelpText( wxT_2( "Panel with a maximized button" ) );
#endif

    wxButton *buttonBig = new wxButton(panel, wxID_ANY, wxT_2("Maximized button"));
...
#if wxUSE_HELP
    panel->SetHelpText( wxT_2( "Maroon panel" ) );
#endif

    panel->SetBackgroundColour( wxColour( wxT_2("MAROON") ) );
    (void) new wxStaticText( panel, wxID_ANY,
                             wxT_2("This page has been inserted, not added."),
                             wxPoint(10, 10) );
...
wxPanel *CreatePage(wxBookCtrlBase *parent, const wxString&pageName)
{
    ...
    wxFAIL_MSG( wxT_2("unknown page name") );
...
MyFrame::MyFrame()
    : wxFrame(NULL, wxID_ANY, wxString(wxT_2("wxWidgets book controls sample")))
{
#if wxUSE_HELP
...
#if wxUSE_NOTEBOOK
    menuType->AppendRadioItem(ID_BOOK_NOTEBOOK,   wxT_2("&Notebook\tCtrl-1"));
#endif
#if wxUSE_LISTBOOK
    menuType->AppendRadioItem(ID_BOOK_LISTBOOK,   wxT_2("&Listbook\tCtrl-2"));
#endif
#if wxUSE_CHOICEBOOK
    menuType->AppendRadioItem(ID_BOOK_CHOICEBOOK, wxT_2("&Choicebook\tCtrl-3"));
#endif
#if wxUSE_TREEBOOK
    menuType->AppendRadioItem(ID_BOOK_TREEBOOK,   wxT_2("&Treebook\tCtrl-4"));
#endif
#if wxUSE_TOOLBOOK
    menuType->AppendRadioItem(ID_BOOK_TOOLBOOK,   wxT_2("T&oolbook\tCtrl-5"));
#endif
#if wxUSE_AUI
    menuType->AppendRadioItem(ID_BOOK_AUINOTEBOOK,   wxT_2("&AuiNotebook\tCtrl-6"));
#endif
    ...
    menuOrient->AppendRadioItem(ID_ORIENT_DEFAULT, wxT_2("&Default\tAlt-0"));
    menuOrient->AppendRadioItem(ID_ORIENT_TOP,     wxT_2("&Top\tAlt-1"));
    menuOrient->AppendRadioItem(ID_ORIENT_BOTTOM,  wxT_2("&Bottom\tAlt-2"));
    menuOrient->AppendRadioItem(ID_ORIENT_LEFT,    wxT_2("&Left\tAlt-3"));
    menuOrient->AppendRadioItem(ID_ORIENT_RIGHT,   wxT_2("&Right\tAlt-4"));

    wxMenu *menuStyle = new wxMenu;
#if wxUSE_NOTEBOOK
    menuStyle->AppendCheckItem(ID_FIXEDWIDTH, wxT_2("&Fixed Width (wxNotebook)"));
    menuStyle->AppendCheckItem(ID_MULTI, wxT_2("&Multiple lines (wxNotebook)"));
    menuStyle->AppendCheckItem(ID_NOPAGETHEME, wxT_2("&No Page Theme (wxNotebook)"));
#endif
#if wxUSE_TOOLBOOK
    menuStyle->AppendCheckItem(ID_BUTTONBAR, wxT_2("&Button Bar (wxToolbook)"));
    menuStyle->AppendCheckItem(ID_HORZ_LAYOUT, wxT_2("&Horizontal layout (wxToolbook)"));
#endif

    wxMenu *menuPageOperations = new wxMenu;
    menuPageOperations->Append(ID_ADD_PAGE, wxT_2("&Add page\tAlt-A"));
    menuPageOperations->Append(ID_ADD_PAGE_NO_SELECT, wxT_2("&Add page (don't select)\tAlt-B"));
    menuPageOperations->Append(ID_INSERT_PAGE, wxT_2("&Insert page\tAlt-I"));
    menuPageOperations->Append(ID_DELETE_CUR_PAGE, wxT_2("&Delete current page\tAlt-D"));
    menuPageOperations->Append(ID_DELETE_LAST_PAGE, wxT_2("D&elete last page\tAlt-L"));
    menuPageOperations->Append(ID_NEXT_PAGE, wxT_2("&Next page\tAlt-N"));
#if wxUSE_TREEBOOK
    menuPageOperations->AppendSeparator();
    menuPageOperations->Append(ID_ADD_PAGE_BEFORE, wxT_2("Insert page &before\tAlt-B"));
    menuPageOperations->Append(ID_ADD_SUB_PAGE, wxT_2("Add s&ub page\tAlt-U"));
#endif
    menuPageOperations->AppendSeparator();
    menuPageOperations->Append(ID_CHANGE_SELECTION, wxT_2("&Change selection to 0\tCtrl-0"));
    menuPageOperations->Append(ID_SET_SELECTION, wxT_2("&Set selection to 0\tShift-Ctrl-0"));
    menuPageOperations->AppendSeparator();
...
#if wxUSE_HELP
    menuOperations->Append(ID_CONTEXT_HELP, wxT_2("&Context help\tCtrl-F1"));
#endif // wxUSE_HELP
    menuOperations->Append(ID_HITTEST, wxT_2("&Hit test\tCtrl-H"));

    wxMenu *menuFile = new wxMenu;
    menuFile->Append(wxID_ANY, wxT_2("&Type"), menuType, wxT_2("Type of control"));
    menuFile->Append(wxID_ANY, wxT_2("&Orientation"), menuOrient, wxT_2("Orientation of control"));
    menuFile->AppendCheckItem(ID_SHOW_IMAGES, wxT_2("&Show images\tAlt-S"));
    menuFile->Append(wxID_ANY, wxT_2("&Style"), menuStyle, wxT_2("Style of control"));
    menuFile->AppendSeparator();
    menuFile->Append(wxID_EXIT, wxT_2("E&xit"), wxT_2("Quits the application"));
    menuFile->Check(ID_SHOW_IMAGES, m_chkShowImages);

    wxMenuBar *menuBar = new wxMenuBar;
    menuBar->Append(menuFile, wxT_2("&File"));
    menuBar->Append(menuPageOperations, wxT_2("&Pages"));
    menuBar->Append(menuOperations, wxT_2("&Operations"));
    SetMenuBar(menuBar);
...
#define DISPATCH_ON_TYPE(before, nb, lb, cb, tb, toolb, aui, sb, after)       \
    ...
    {                                                                         \
        ...
        CASE_SIMPLEBOOK(before sb after)                                      \
                                                                              \
        default:                                                              \
            wxFAIL_MSG( wxT_2("unknown book control type") );                   \
    }
...
#if wxUSE_HELP
    panel->SetHelpText( wxT_2( "Panel with \"First\" and \"Second\" buttons" ) );
#endif

    (void) new wxButton(panel, wxID_ANY, wxT_2("First button"), wxPoint(10, 30));
    (void) new wxButton(panel, wxID_ANY, wxT_2("Second button"), wxPoint(150, 30));
...
void MyFrame::OnHitTest(wxCommandEvent& WXUNUSED(event))
{
    ...
    AddFlagStrIfFlagPresent( flagsStr, flags, wxBK_HITTEST_NOWHERE, wxS("wxBK_HITTEST_NOWHERE") );
    AddFlagStrIfFlagPresent( flagsStr, flags, wxBK_HITTEST_ONICON,  wxS("wxBK_HITTEST_ONICON") );
    AddFlagStrIfFlagPresent( flagsStr, flags, wxBK_HITTEST_ONLABEL, wxS("wxBK_HITTEST_ONLABEL") );
    AddFlagStrIfFlagPresent( flagsStr, flags, wxBK_HITTEST_ONITEM,  wxS("wxBK_HITTEST_ONITEM") );
    AddFlagStrIfFlagPresent( flagsStr, flags, wxBK_HITTEST_ONPAGE,  wxS("wxBK_HITTEST_ONPAGE") );

    wxLogMessage(wxT_2("HitTest at (%d,%d): %d: %s"),
                 pt.x,
                 pt.y,
                 pagePos,
                 flagsStr.c_str());
...
void MyFrame::OnAddSubPage(wxCommandEvent& WXUNUSED(event))
{
    ...
    if ( currBook )
    {
        ...
        {
            wxLogError(wxT_2("Select the parent page first!"));
            return;
        }
...
void MyFrame::OnAddPageBefore(wxCommandEvent& WXUNUSED(event))
{
    ...
    if ( currBook )
    {
        ...
        currBook->InsertPage(selPos,
                             ...
                             wxString::Format
                             (
                                ADDED_PAGE_NAME_BEFORE wxT_2("%u"),
                                ++s_subPageAdded
                             ),
                             true,
                             GetIconIndex(currBook));
...
void MyFrame::OnInsertPage(wxCommandEvent& WXUNUSED(event))
{
    ...
    if ( currBook )
    {
        ...
        currBook->InsertPage( 0, panel,
            wxString::Format(INSERTED_PAGE_NAME wxT_2("%u"), ++s_pageIns), false,
            GetIconIndex(currBook) );
...
void MyFrame::OnIdle( wxIdleEvent& WXUNUSED(event) )
{
    ...
    if ( nPages != s_nPages || nSel != s_nSel || s_currBook != currBook )
    {
        ...
        wxString selection;
        if ( nSel == wxNOT_FOUND )
            selection << wxT_2("not found");
        else
            selection << nSel;

        wxString title;
        title.Printf(wxT_2("Notebook and friends (%d pages, selection: %s)"), nPages, selection.c_str());
...
void MyFrame::OnBookCtrl(wxBookCtrlBaseEvent& event)
{
    ...
    for ( size_t n = 0; n < WXSIZEOF(events); n++ )
    {
        ...
        else if ( eventType == ei.typeChanging )
        {
                    ...
                    book && book->GetPageText(idx) == VETO_PAGE_NAME )
            {
                if ( wxMessageBox
                     (
                      wxT_2("Are you sure you want to leave this page?\n")
                      wxT_2("(This demonstrates veto-ing)"),
                      wxT_2("Notebook sample"),
    ...
    wxLogMessage(wxT_2("Event #%d: %s: %s (%d) new sel %d, old %d, current %d%s"),
                 ++s_num,
                 nameControl.c_str(),
                 nameEvent.c_str(),
                 eventType,
