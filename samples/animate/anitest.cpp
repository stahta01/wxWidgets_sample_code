...
bool MyApp::OnInit()
{
    ...
    MyFrame* frame = new MyFrame((wxFrame *)NULL, wxID_ANY, wxT_2("Animation Demo"),
                          ...
                          style | wxNO_FULL_REPAINT_ON_RESIZE)
{
...
#if wxUSE_FILEDLG
    file_menu->Append(wxID_OPEN, wxT_2("&Open Animation...\tCtrl+O"), wxT_2("Loads an animation"));
#endif // wxUSE_FILEDLG
    file_menu->Append(wxID_EXIT);

    wxMenu *play_menu = new wxMenu;
    play_menu->Append(ID_PLAY, wxT_2("Play\tCtrl+P"), wxT_2("Play the animation"));
    play_menu->Append(wxID_STOP, wxT_2("Stop\tCtrl+S"), wxT_2("Stop the animation"));
    play_menu->AppendSeparator();
    play_menu->Append(ID_SET_NULL_ANIMATION, wxT_2("Set null animation"),
                      wxT_2("Sets the empty animation in the control"));
    play_menu->AppendCheckItem(ID_SET_INACTIVE_BITMAP, wxT_2("Set inactive bitmap"),
                               wxT_2("Sets an inactive bitmap for the control"));
    play_menu->AppendCheckItem(ID_SET_NO_AUTO_RESIZE, wxT_2("Set no autoresize"),
                               wxT_2("Tells the control not to resize automatically"));
    play_menu->Append(ID_SET_BGCOLOR, wxT_2("Set background colour..."),
                      wxT_2("Sets the background colour of the control"));
    ...
    menu_bar->Append(play_menu, wxT_2("&Animation"));
    ...
    sz->Add(new wxStaticText(this, wxID_ANY, wxT_2("wxAnimationCtrl:")),
            wxSizerFlags().Centre().Border());

    m_animationCtrl = new wxAnimationCtrl(this, wxID_ANY);
    if (m_animationCtrl->LoadFile(wxT_2("throbber.gif")))
...
void MyFrame::OnPlay(wxCommandEvent& WXUNUSED(event))
{
    ...
    {
        wxLogError(wxT_2("Invalid animation"));
    }
}
...
void MyFrame::OnSetBgColor(wxCommandEvent& WXUNUSED(event))
{
    wxColour clr = wxGetColourFromUser(this, m_animationCtrl->GetBackgroundColour(),
                                       wxT_2("Choose the background colour"));
...
void MyFrame::OnAbout(wxCommandEvent& WXUNUSED(event) )
{
    ...
    info.SetCopyright(wxT_2("(C) 2006 Julian Smart"));

    info.AddDeveloper(wxT_2("Julian Smart"));
    info.AddDeveloper(wxT_2("Guillermo Rodriguez Garcia"));
    info.AddDeveloper(wxT_2("Francesco Montorsi"));
...
{
    wxFileDialog dialog(this, wxT_2("Please choose an animation"),
                        wxEmptyString, wxEmptyString, wxT_2("*.gif;*.ani"), wxFD_OPEN);
    if (dialog.ShowModal() == wxID_OK)
    {
        ...
        if (m_animationCtrl->LoadFile(filename))
            ...
            wxMessageBox(wxT_2("Sorry, this animation is not a valid format for wxAnimation."));
#else
    #if 0
        wxAnimation temp;
        if (!temp.LoadFile(filename))
        {
            wxLogError(wxT_2("Sorry, this animation is not a valid format for wxAnimation."));
            return;
        }
