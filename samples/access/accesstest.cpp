...
bool MyApp::OnInit()
{
    ...
    MyFrame *frame = new MyFrame(wxT_2("AccessTest wxWidgets App"),
    ...
    return true;
#else
    wxMessageBox( wxT_2("This sample has to be compiled with wxUSE_ACCESSIBILITY"), wxT_2("Building error"), wxOK);
    return false;
#endif // wxUSE_ACCESSIBILITY
}
...
public:
    ...
    virtual wxAccStatus GetFocus(int* childId, wxAccessible** child);
        ...
        // - a list variant (GetType() == wxT_2("list"))
        // - an integer representing the selected child element,
        //   or 0 if this object is selected (GetType() == wxT_2("long"))
       ...
       : wxFrame(NULL, wxID_ANY, title, pos, size, style)
{
    ...
    helpMenu->Append(AccessTest_About, wxT_2("&About"), wxT_2("Show about dialog"));

    menuFile->Append(AccessTest_Query, wxT_2("Query"), wxT_2("Query the window hierarchy"));
    menuFile->AppendSeparator();
    menuFile->Append(AccessTest_Quit, wxT_2("E&xit\tAlt-X"), wxT_2("Quit this program"));
    ...
    listBox->Append(wxT_2("Cabbages"));
    listBox->Append(wxT_2("Kings"));
    listBox->Append(wxT_2("Sealing wax"));
    listBox->Append(wxT_2("Strings"));
    listBox->CreateAccessible();
    listBox->SetHelpText(wxT_2("This is a sample wxWidgets listbox, with a number of items in it."));

    m_textCtrl = new wxTextCtrl(splitter, wxID_ANY, wxT_2(""), wxDefaultPosition,
        wxDefaultSize, wxTE_MULTILINE);
    m_textCtrl->CreateAccessible();
    m_textCtrl->SetHelpText(wxT_2("This is a sample wxWidgets multiline text control."));
...
{
    wxString msg;
    msg.Printf( wxT_2("This is the About dialog of the AccessTest sample.\n")
                wxT_2("Welcome to %s"), wxVERSION_STRING);

    wxMessageBox(msg, wxT_2("About AccessTest"), wxOK | wxICON_INFORMATION, this);
}
...
void MyFrame::OnQuery(wxCommandEvent& WXUNUSED(event))
{
    ...
    {
        Log(wxT_2("Could not get object."));
        return;
    }
    if (accessibleFrame)
    {
        //Log(wxT_2("Got an IAccessible for the frame."));
        LogObject(0, accessibleFrame);
        Log(wxT_2("Checking children using AccessibleChildren()..."));

        // Now check the AccessibleChildren function works OK
        long childCount = 0;
        if (S_OK != accessibleFrame->get_accChildCount(& childCount))
        {
            Log(wxT_2("Could not get number of children."));
            accessibleFrame->Release();
            return;
        }
        else if (childCount == 0)
        {
            Log(wxT_2("No children."));
            accessibleFrame->Release();
            return;
        }
            ...
            for (i = 0; i < childCount; i++)
            {
                ...
                if (var[i].pdispVal)
                {
                    if (var[i].pdispVal->QueryInterface(IID_IAccessible, (LPVOID*) & childAccessible) == S_OK)
                    {
                        ...
                        wxString str;
                        str.Printf(wxT_2("Found child %s/%s"), name.c_str(), role.c_str());
                        Log(str);
        ...
        {
            Log(wxT_2("AccessibleChildren failed."));
        }
        delete[] var;
...
void MyFrame::Log(const wxString& text)
{
    if (m_textCtrl)
    {
        wxString text2(text);
        text2.Replace(wxT_2("\n"), wxT_2(" "));
        text2.Replace(wxT_2("\r"), wxT_2(" "));
        m_textCtrl->SetInsertionPointEnd();
        m_textCtrl->WriteText(text2 + wxT_2("\n"));
    }
}
...
void MyFrame::LogObject(int indent, IAccessible* obj)
{
    ...
    if (indent == 0)
    {
        ...
        wxString str;
        str.Printf(wxT_2("Name = %s; Role = %s"), name.c_str(), role.c_str());
        str.Pad(indent, wxT_2(' '), false);
        Log(str);
    }
    ...
    {
        wxString str;
        str.Printf(wxT_2("There are %d children."), (int) childCount);
        str.Pad(indent, wxT_2(' '), false);
        Log(str);
        Log(wxT_2(""));
    }
    ...
    for (i = 1; i <= childCount; i++)
    {
        ...
        wxString str;
        str.Printf(wxT_2("%d) Name = %s; Role = %s"), i, name.c_str(), role.c_str());
        str.Pad(indent, wxT_2(' '), false);
        Log(str);
        ...
        {
            str.Printf(wxT_2("This is a real object."));
            str.Pad(indent+4, wxT_2(' '), false);
            Log(str);
        ...
        {
            str.Printf(wxT_2("This is an element."));
            str.Pad(indent+4, wxT_2(' '), false);
            Log(str);
        }
        // Log(wxT_2(""));
