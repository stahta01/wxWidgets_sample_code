    ...
    , m_iconSmileXpm(smile_xpm)
{
    ...
    wxString dir;
    if ( wxFile::Exists(wxT_2("./horse.png")) )
        dir = wxT_2("./");
    else if ( wxFile::Exists(wxT_2("../horse.png")) )
        dir = wxT_2("../");
    else
        wxLogWarning(wxT_2("Can't find image files in either '.' or '..'!"));

    wxImage image = bitmap.ConvertToImage();

#if wxUSE_LIBPNG
    if ( !image.SaveFile( dir + wxT_2("test.png"), wxBITMAP_TYPE_PNG ))
    {
        wxLogError(wxT_2("Can't save file"));
    }

    image.Destroy();

    if ( image.LoadFile( dir + wxT_2("test.png") ) )
        my_square = wxBitmap( image );

    image.Destroy();

    if ( !image.LoadFile( dir + wxT_2("horse.png")) )
    {
        wxLogError(wxT_2("Can't load PNG image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("toucan.png")) )
    {
        wxLogError(wxT_2("Can't load PNG image"));
    }
    else
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.jpg")) )
    {
        wxLogError(wxT_2("Can't load JPG image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("cmyk.jpg")) )
    {
        wxLogError(wxT_2("Can't load CMYK JPG image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.gif" )) )
    {
        wxLogError(wxT_2("Can't load GIF image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.pcx"), wxBITMAP_TYPE_PCX ) )
    {
        wxLogError(wxT_2("Can't load PCX image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.bmp"), wxBITMAP_TYPE_BMP ) )
    {
        wxLogError(wxT_2("Can't load BMP image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.xpm"), wxBITMAP_TYPE_XPM ) )
    {
        wxLogError(wxT_2("Can't load XPM image"));
    }
    else
    {
    ...
    if ( !image.SaveFile( dir + wxT_2("test.xpm"), wxBITMAP_TYPE_XPM ))
    {
        wxLogError(wxT_2("Can't save file"));
    }
#endif
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.pnm"), wxBITMAP_TYPE_PNM ) )
    {
        wxLogError(wxT_2("Can't load PNM image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("horse_ag.pnm"), wxBITMAP_TYPE_PNM ) )
    {
        wxLogError(wxT_2("Can't load PNM image"));
    }
    else
    ...
    if ( !image.LoadFile( dir + wxT_2("horse_rg.pnm"), wxBITMAP_TYPE_PNM ) )
    {
        wxLogError(wxT_2("Can't load PNM image"));
    }
    else
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.tif"), wxBITMAP_TYPE_TIFF ) )
    {
        wxLogError(wxT_2("Can't load TIFF image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.tga"), wxBITMAP_TYPE_TGA ) )
    {
        wxLogError(wxT_2("Can't load TGA image"));
    }
    else
    {
    ...
    if ( m_bmpSmileXpm.IsOk() )
        m_bmpSmileXpm.SaveFile(wxT_2("saved.xpm"), wxBITMAP_TYPE_XPM);

#if wxUSE_ICO_CUR
    image.Destroy();

    if ( !image.LoadFile( dir + wxT_2("horse.ico"), wxBITMAP_TYPE_ICO, 0 ) )
    {
        wxLogError(wxT_2("Can't load first ICO image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.ico"), wxBITMAP_TYPE_ICO, 1 ) )
    {
        wxLogError(wxT_2("Can't load second ICO image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.ico") ) )
    {
        wxLogError(wxT_2("Can't load best ICO image"));
    }
    else
    {
    ...
    if ( !image.LoadFile( dir + wxT_2("horse.cur"), wxBITMAP_TYPE_CUR ) )
    {
        wxLogError(wxT_2("Can't load best ICO image"));
    }
    else
    ...
    m_ani_images = wxImage::GetImageCount ( dir + wxT_2("horse3.ani"), wxBITMAP_TYPE_ANI );
    if (m_ani_images==0)
    {
        wxLogError(wxT_2("No ANI-format images found"));
    }
    else
    {
    ...
    {
        image.Destroy();
        if (!image.LoadFile( dir + wxT_2("horse3.ani"), wxBITMAP_TYPE_ANI, i ))
        {
            wxString tmp = wxT_2("Can't load image number ");
            tmp << i ;
            wxLogError(tmp);
        }
        else
    ...
    wxFile file(dir + wxT_2("horse.bmp"));
    if ( file.IsOpened() )
    {
        ...
        {
            wxLogError(wxT_2("Reading bitmap file failed"));
        }
        else
        {
            ...
            {
                wxLogError(wxT_2("Can't load BMP image from stream"));
            }
            else
            {
...
void MyCanvas::OnPaint( wxPaintEvent &WXUNUSED(event) )
{
    ...
    dc.DrawText( wxT_2("Loaded image"), 30, 10 );
    if (my_square.IsOk())
        dc.DrawBitmap( my_square, 30, 30 );

    dc.DrawText( wxT_2("Drawn directly"), 150, 10 );
    dc.SetBrush( wxBrush( wxS("orange") ) );
    dc.SetPen( *wxBLACK_PEN );
    ...
    // toucans
    {
        ...
        dc.DrawText(wxT_2("Original toucan"), x+50, y);
        dc.DrawBitmap(my_toucan, x, y+15, true);
        y += yy;
        dc.DrawText(wxT_2("Flipped horizontally"), x+50, y);
        dc.DrawBitmap(my_toucan_flipped_horiz, x, y+15, true);
        y += yy;
        dc.DrawText(wxT_2("Flipped vertically"), x+50, y);
        dc.DrawBitmap(my_toucan_flipped_vert, x, y+15, true);
        y += yy;
        dc.DrawText(wxT_2("Flipped both h&v"), x+50, y);
        dc.DrawBitmap(my_toucan_flipped_both, x, y+15, true);

        y += yy;
        dc.DrawText(wxT_2("In greyscale"), x+50, y);
        dc.DrawBitmap(my_toucan_grey, x, y+15, true);

        y += yy;
        dc.DrawText(wxT_2("Toucan's head"), x+50, y);
        dc.DrawBitmap(my_toucan_head, x, y+15, true);

        y += yy;
        dc.DrawText(wxT_2("Scaled with normal quality"), x+50, y);
        dc.DrawBitmap(my_toucan_scaled_normal, x, y+15, true);

        y += yy;
        dc.DrawText(wxT_2("Scaled with high quality"), x+50, y);
        dc.DrawBitmap(my_toucan_scaled_high, x, y+15, true);

        y += yy;
        dc.DrawText(wxT_2("Blured"), x+50, y);
    ...
    if (my_smile_xbm.IsOk())
    {
        ...
        dc.DrawText( wxT_2("XBM bitmap"), x, y );
        dc.DrawText( wxT_2("(green on red)"), x, y + 15 );
        dc.SetTextForeground( wxT_2("GREEN") );
        dc.SetTextBackground( wxT_2("RED") );
        dc.DrawBitmap( my_smile_xbm, x, y + 30 );

        dc.SetTextForeground( *wxBLACK );
        dc.DrawText( wxT_2("After wxImage conversion"), x + 120, y );
        dc.DrawText( wxT_2("(red on white)"), x + 120, y + 15 );
        dc.SetTextForeground( wxT_2("RED") );
    ...
    if (mono.IsOk())
    {
        ...
        dc.DrawText( wxT_2("Mono bitmap"), x, y );
        dc.DrawText( wxT_2("(red on green)"), x, y + 15 );
        dc.SetTextForeground( wxT_2("RED") );
        dc.SetTextBackground( wxT_2("GREEN") );
    ...
    dc.DrawText(wxT_2("XPM bitmap"), 30, 2230 );
    if ( m_bmpSmileXpm.IsOk() )
        dc.DrawBitmap(m_bmpSmileXpm, 30, 2250, true);

    dc.DrawText(wxT_2("XPM icon"), 110, 2230 );
    ...
    if (to_blit.IsOk())
    {
        ...
        dc.DrawText( wxT_2("Blit"), 400, 2230);
        wxMemoryDC blit_dc;
    ...
    dc.DrawText( wxT_2("ANI handler"), 230, 2390 );
...
void MyCanvas::CreateAntiAliasedBitmap()
{
...

    {
        ...
        dc.SetTextForeground( wxT_2("RED") );
        dc.DrawText( wxT_2("This is anti-aliased Text."), 20, 5 );
        dc.DrawText( wxT_2("And a Rectangle."), 20, 45 );
