...
bool MyApp::OnInit()
{
    ...
    MyFrame *frame = new MyFrame(NULL, wxT_2("wxWidgets Penguin Sample"),
        wxDefaultPosition, wxDefaultSize);

#if wxUSE_ZLIB
    if (wxFileExists(wxT_2("penguin.dxf.gz")))
        frame->GetCanvas()->LoadDXF(wxT_2("penguin.dxf.gz"));
#else
    if (wxFileExists(wxT_2("penguin.dxf")))
        frame->GetCanvas()->LoadDXF(wxT_2("penguin.dxf"));
#endif
    ...
    : wxFrame(frame, wxID_ANY, title, pos, size, style)
{
    ...
    fileMenu->Append(wxID_OPEN, wxT_2("&Open..."));
    fileMenu->AppendSeparator();
    fileMenu->Append(wxID_EXIT, wxT_2("E&xit\tALT-X"));
    // Make the "Help" menu
    wxMenu *helpMenu = new wxMenu;
    helpMenu->Append(wxID_HELP, wxT_2("&About"));
...
{
    wxString filename = wxFileSelector(wxT_2("Choose DXF Model"), wxT_2(""), wxT_2(""), wxT_2(""),
#if wxUSE_ZLIB
        wxT_2("DXF Drawing (*.dxf;*.dxf.gz)|*.dxf;*.dxf.gz|All files (*.*)|*.*"),
#else
        wxT_2("DXF Drawing (*.dxf)|*.dxf)|All files (*.*)|*.*"),
#endif
        wxFD_OPEN);
    if (!filename.IsEmpty())
    {
...
{
    wxMessageBox(wxT_2("OpenGL Penguin Sample (c) Robert Roebling, Sandro Sigala et al"));
}
...
#if wxUSE_ZLIB
    {
        if (filename.Right(3).Lower() == wxT_2(".gz"))
        {
