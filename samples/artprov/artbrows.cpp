...
#define ART_CLIENT(id) \
    choice->Append(wxT_2(#id), (void*)id);
#define ART_ICON(id) \
    { \
        int ind; \
        ...
        if ( icon.IsOk() ) \
            ...
            ind = 0; \
        list->InsertItem(index, wxT_2(#id), ind); \
...
wxArtBrowserDialog::wxArtBrowserDialog(wxWindow *parent)
    : wxDialog(parent, wxID_ANY, wxT_2("Art resources browser"),
               ...
               wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER)
{
    ...
    subsizer->Add(new wxStaticText(this, wxID_ANY, wxT_2("Client:")), 0, wxALIGN_CENTER_VERTICAL);
    ...
    m_list->AppendColumn(wxT_2("wxArtID"));
    subsizer->Add(m_list, 1, wxEXPAND | wxRIGHT, 10);

    wxSizer *subsub = new wxBoxSizer(wxVERTICAL);
    m_text = new wxStaticText(this, wxID_ANY, wxT_2("Size: 333x333"));
    subsub->Add(m_text);
    ...
    wxButton *ok = new wxButton(this, wxID_OK, wxT_2("Close"));
    ok->SetDefault();
...
void wxArtBrowserDialog::SetArtBitmap(const wxArtID& id, const wxArtClient& client, const wxSize& size)
{
    ...
    m_text->SetLabel(wxString::Format(wxT_2("Size: %d x %d"), bmp.GetWidth(), bmp.GetHeight()));
    Refresh();
}
